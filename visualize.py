...
def apply_kp_mask(image, mask, color,kp_class_ids, kp_class_names,
        ax,alpha=0.5, mode=2):
    """Apply the given mask to the image.
    """
    ...
    for i in range(mask.shape[0]):
        ...
        wd = patches.Wedge((ix[1][0],ix[0][0]), 4, 0, 360, width=1.6,\
                color="black")
        cc = patches.Circle((ix[1][0],ix[0][0]), 2,\
                color="g")
        ax.add_patch(wd)
        ax.add_patch(cc)
    return image
...
def display_kp(image, boxes, masks, class_ids, kp_class_names,
                      kp_class_ids,
                      scores=None, title="",
    ...
    if not N:
        ...
        print(boxes.shape[0], masks.shape[0])
...
def display_only_kp(image, masks, titles=None, cols=4, cmap=None,
    ...
    # image:[H,W,C]
    # masks:[NUM_KPS, H_M, W_M, NUM_INSTANCE]
    N = masks.shape[-1]
    ...
    boxes = utils.extract_bboxes_by_kps(masks,image.shape,20)
    print("boxes")
    print(boxes)
    ...
    for j in range(N):
        mask = masks[:,:, :, j]
        image = apply_kp_mask(image, mask, (1.0,1.0,1.0))
    ax.imshow(image.astype(np.uint8), cmap=cmap,
                norm=norm, interpolation=interpolation)
    plt.show()
